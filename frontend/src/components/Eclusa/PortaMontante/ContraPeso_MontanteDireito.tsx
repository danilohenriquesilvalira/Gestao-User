...
interface ContraPesoMontanteDireitoProps {
  editMode?: boolean;
  componentId?: string;
  nivel?: number; // Prop para receber o valor do WebSocket (0-100)
}

const ContraPeso_MontanteDireito: React.FC<ContraPesoMontanteDireitoProps> = ({ 
  editMode = false,
  componentId = 'contrapeso-montante-direito',
  nivel: nivelExterno 
}) => {
  // Estado interno para quando não recebe o valor via prop
  const [nivelInterno, setNivelInterno] = useState(0); // Estado inicial: 0% (posição inicial)

  // Usa o valor externo se fornecido, senão usa o interno
  const nivel = nivelExterno !== undefined ? nivelExterno : nivelInterno;

  const handleNivelChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    setNivelInterno(Number(event.target.value));
  };

  // Altura da linha que se estende acima do contrapeso
  const linhaHeight = nivel * 6; // Multiplicador para o movimento
  
  // Altura do SVG dinâmica - sempre garante espaço suficiente
  const svgHeight = Math.max(800, linhaHeight + 300); // Mais espaço para evitar cortes
  
  // Altura total da linha (sempre conecta ao contrapeso)
  const alturaLinhaTotal = linhaHeight + 136; // 136 é a altura do contrapeso
  ...
  return (
    <ResponsiveWrapper
      ...
      editMode={editMode}
      defaultConfig={{
        xs: { x: 600, y: 150, width: 120, height: 400, scale: 0.6, zIndex: 7, opacity: 1, rotation: 0 },
        sm: { x: 700, y: 200, width: 140, height: 450, scale: 0.65, zIndex: 7, opacity: 1, rotation: 0 },
        md: { x: 800, y: 250, width: 160, height: 500, scale: 0.7, zIndex: 7, opacity: 1, rotation: 0 },
        lg: { x: 900, y: 300, width: 180, height: 550, scale: 0.75, zIndex: 7, opacity: 1, rotation: 0 },
        xl: { x: 1000, y: 350, width: 200, height: 600, scale: 0.8, zIndex: 7, opacity: 1, rotation: 0 },
        '2xl': { x: 1200, y: 400, width: 220, height: 650, scale: 0.85, zIndex: 7, opacity: 1, rotation: 0 },
        '3xl': { x: 1400, y: 450, width: 240, height: 700, scale: 0.9, zIndex: 7, opacity: 1, rotation: 0 },
        '4xl': { x: 1600, y: 500, width: 260, height: 750, scale: 0.95, zIndex: 7, opacity: 1, rotation: 0 }
      }}
    >
      <div className={`w-full h-full flex flex-col items-center justify-center ${editMode ? 'border-2 border-green-500 bg-green-50/20' : ''}`}>
        ...
        <div className="relative w-full h-full flex items-center gap-4">
          <div className="relative w-full h-full">
            ...
            >
              {/* Linha preta que se estende para cima - sempre conecta ao contrapeso */}
              <rect 
                x="42.5" 
                y="0" 
                width="5" 
                height={alturaLinhaTotal} 
                fill="black"
              ...
              >
                ...
                <defs>
                  <linearGradient id="paint0_linear_direito" x1="90.5005" y1="128.979" x2="0.000210953" y2="129.008" gradientUnits="userSpaceOnUse">
                    <stop stopColor="#808080"/>
                    <stop offset="0.5" stopColor="white"/>
                    <stop offset="1" stopColor="#808080"/>
                  </linearGradient>
                  <linearGradient id="paint1_linear_direito" x1="90.5005" y1="127.967" x2="0.000210953" y2="127.996" gradientUnits="userSpaceOnUse">
                    <stop stopColor="#808080"/>
                    <stop offset="0.5" stopColor="white"/>
                    <stop offset="1" stopColor="#808080"/>
                  </linearGradient>
                  <linearGradient id="paint2_linear_direito" x1="90.4964" y1="22.624" x2="-3.29304e-07" y2="22.624" gradientUnits="userSpaceOnUse">
                    <stop stopColor="#808080"/>
                    <stop offset="0.5" stopColor="white"/>
                    <stop offset="1" stopColor="#808080"/>
                  </linearGradient>
                  <linearGradient id="paint3_linear_direito" x1="91" y1="14.0973" x2="0" y2="14.0973" gradientUnits="userSpaceOnUse">
                    <stop stopColor="#808080"/>
                    <stop offset="0.5" stopColor="white"/>
                    <stop offset="1" stopColor="#808080"/>
                  </linearGradient>
          ...
          {nivelExterno === undefined && (
            <div className="flex flex-col items-center">
              <span className="text-black font-bold mb-1">Nível Dir</span>
              <input
                type="range"
                min="0"
                max="100"
              ...
              />
              <span className="text-black mt-2 font-bold">{nivelInterno}%</span>
            </div>
          )}
        </div>

        {editMode && (
          <div className="absolute inset-0 border-2 border-dashed border-green-400 bg-green-50/20 rounded flex items-center justify-center pointer-events-none">
          </div>
        )}
      </div>
    </ResponsiveWrapper>
...
export default ContraPeso_MontanteDireito;
