...
interface ContraPesoMontanteProps {
  editMode?: boolean;
  componentId?: string;
  nivel?: number; // Prop para receber o valor do WebSocket (0-100)
}

const ContraPeso_Montante: React.FC<ContraPesoMontanteProps> = ({ 
  editMode = false,
  componentId = 'contrapeso-montante',
  nivel: nivelExterno 
}) => {
  ...
  const linhaHeight = nivel * 4; // Multiplicador para controlar o movimento
  ...
  return (
    <ResponsiveWrapper
      ...
      editMode={editMode}
      defaultConfig={{
        xs: { x: 100, y: 150, width: 120, height: 400, scale: 0.6, zIndex: 7, opacity: 1, rotation: 0 },
        sm: { x: 150, y: 200, width: 140, height: 450, scale: 0.65, zIndex: 7, opacity: 1, rotation: 0 },
        md: { x: 200, y: 250, width: 160, height: 500, scale: 0.7, zIndex: 7, opacity: 1, rotation: 0 },
        lg: { x: 250, y: 300, width: 180, height: 550, scale: 0.75, zIndex: 7, opacity: 1, rotation: 0 },
        xl: { x: 300, y: 350, width: 200, height: 600, scale: 0.8, zIndex: 7, opacity: 1, rotation: 0 },
        '2xl': { x: 350, y: 400, width: 220, height: 650, scale: 0.85, zIndex: 7, opacity: 1, rotation: 0 },
        '3xl': { x: 400, y: 450, width: 240, height: 700, scale: 0.9, zIndex: 7, opacity: 1, rotation: 0 },
        '4xl': { x: 450, y: 500, width: 260, height: 750, scale: 0.95, zIndex: 7, opacity: 1, rotation: 0 }
      }}
    >
      <div className={`w-full h-full flex flex-col items-center justify-center ${editMode ? 'border-2 border-blue-500 bg-blue-50/20' : ''}`}>
        ...
        <div className="relative flex items-center gap-4">
          ...
          {nivelExterno === undefined && (
            <div className="flex flex-col items-center">
              <span className="text-black font-bold mb-1">Nível</span>
              <input
                type="range"
                min="0"
                max="100"
        ...
        {editMode && (
          <div className="absolute inset-0 border-2 border-dashed border-orange-400 bg-orange-50/20 rounded flex items-center justify-center pointer-events-none">
          </div>
        )}
      </div>
    </ResponsiveWrapper>
