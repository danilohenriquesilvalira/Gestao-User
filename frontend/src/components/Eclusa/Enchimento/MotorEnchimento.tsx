...
export interface MotorEnchimentoProps {
...
const MotorEnchimento: React.FC<MotorEnchimentoProps> = ({ 
  ...
  className = '',
  editMode = false,
  componentId = `motor-enchimento-${side}`
}) => {
  ...
  const getDefaultPositions = () => {
    const baseY = 400;
    const leftX = side === 'esquerdo' ? 200 : 1300;
    
    return {
      xs: { x: leftX - 400, y: baseY, width: 60, height: 30, scale: 1, zIndex: 3, opacity: 1, rotation: 0 },
      sm: { x: leftX - 300, y: baseY, width: 65, height: 32, scale: 1, zIndex: 3, opacity: 1, rotation: 0 },
      md: { x: leftX - 200, y: baseY, width: 70, height: 34, scale: 1, zIndex: 3, opacity: 1, rotation: 0 },
      lg: { x: leftX, y: baseY, width: 82, height: 40, scale: 1, zIndex: 3, opacity: 1, rotation: 0 },
      xl: { x: leftX + 100, y: baseY, width: 90, height: 44, scale: 1, zIndex: 3, opacity: 1, rotation: 0 },
      '2xl': { x: leftX + 200, y: baseY, width: 95, height: 46, scale: 1, zIndex: 3, opacity: 1, rotation: 0 },
      '3xl': { x: leftX + 300, y: baseY, width: 100, height: 48, scale: 1, zIndex: 3, opacity: 1, rotation: 0 },
      '4xl': { x: leftX + 400, y: baseY, width: 105, height: 50, scale: 1, zIndex: 3, opacity: 1, rotation: 0 }
    };
  };
  ...
  return (
    ...
    >
      <div className="motor-enchimento-wrapper" style={{ 
        width: '100%', 
        height: '100%',
        position: 'relative',
        display: 'flex',
        ...
        <svg 
          ...
          className={className}
          style={{
            transform: side === 'direito' ? 'scaleX(-1)' : 'none', // Espelha o lado direito
...
// Estados do motor:
// status={0} -> Cinza (inativo)
// status={1} -> Verde (operando)  
// status={2} -> Vermelho (falha)
