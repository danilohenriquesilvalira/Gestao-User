...
type NavItem = 'eclusa' | 'enchimento' | 'porta_jusante' | 'porta_montante' | 'configuracoes' | 'usuarios';
...
      switch (itemId) {
        case 'eclusa':
          window.location.href = '/dashboard'; // Eclusa Principal (Dashboard)
          break;
        case 'configuracoes':
...
        isHovered ? 'translate-x-0' : '-translate-x-[44px]'
      }`}>
        
        {/* 3 Traços quando recuado */}
        <div className={`absolute left-[30px] top-1/2 transform -translate-y-1/2 flex flex-col space-y-2 transition-opacity duration-300 ${
          isHovered ? 'opacity-0' : 'opacity-100'
        }`}>
          <div className="w-6 h-1 bg-green-400 rounded-full shadow-md"></div>
          <div className="w-6 h-1 bg-cyan-400 rounded-full shadow-md"></div>
          <div className="w-6 h-1 bg-purple-400 rounded-full shadow-md"></div>
        </div>

        {/* SVG Container */}
        <div className={`transition-opacity duration-300 ${isHovered ? 'opacity-100' : 'opacity-0'} drop-shadow-lg`}>
          <svg width="74" height="464" viewBox="0 0 74 464" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M35.979 44.5C12.9524 37.7 2.3986 12 0 0V232H73.5V80.5C73.5 57.5 59.0056 51.3 35.979 44.5Z" fill="#131827"/>
            <path d="M35.979 419.5C12.9524 426.3 2.3986 452 0 464L0 232L73.5 232L73.5 383.5C73.5 408.42 59.0981 412.673 36.1996 419.435L35.979 419.5Z" fill="#131827"/>
          </svg>

          {/* Navigation Icons */}
          <div className="absolute inset-0 flex flex-col items-center justify-center space-y-8 py-16">
            
            {/* Eclusa Principal (Dashboard) */}
            <button 
...
            >
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M1 323.5V1H562V323.5H1Z" stroke={getIconColor('porta_montante')} strokeWidth="0.5" transform="scale(0.04) translate(-1, -1)" />
                <path d="M1.7627 1.38574H24.7666L150.876 280.22L151.104 280.724L151.331 280.22L277.425 1.38574H285.575L293.266 18.3906L411.684 280.22L411.912 280.724L412.14 280.22L538.233 1.38574H561.43L561.651 1.44629L416.352 322.75H407.383L400.142 306.738L400.187 306.647L400.139 306.54L281.735 44.7109L281.508 44.207L281.279 44.7109L155.543 322.75H146.664L141.385 311.104L139.33 306.552L1.34668 1.44531L1.7627 1.3291V1.38574Z" fill={getIconColor('porta_montante')} stroke={getIconColor('porta_montante')} strokeWidth="0.5" transform="scale(0.04) translate(-1, -1)" />
              </svg>
            </button>
