...
export const AssistenteVirtual: React.FC = () => {
  ...
  return (
    <>
      {/* Botão Flutuante - Mobile First Responsivo */}
      <div className="fixed bottom-20 xs:bottom-6 right-4 xs:right-6 md:left-6 md:right-auto z-50">
        <button
          onClick={() => setIsOpen(!isOpen)}
          className="bg-blue-600 hover:bg-blue-700 text-white p-3 xs:p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 relative"
        >
          {isConnected && (
            <div className="absolute -top-1 -right-1 w-2.5 h-2.5 xs:w-3 xs:h-3 bg-green-400 rounded-full animate-pulse"></div>
          )}
          {isOpen ? <X className="w-5 h-5 xs:w-6 xs:h-6" /> : <MessageCircle className="w-5 h-5 xs:w-6 xs:h-6" />}
        </button>
      </div>

      {/* Chat Window - Mobile First Responsivo */}
      {isOpen && (
        <div className="fixed inset-x-2 bottom-24 xs:inset-x-4 xs:bottom-20 md:bottom-24 md:left-6 md:right-auto md:w-96 h-[400px] xs:h-[450px] md:h-[500px] bg-white rounded-lg xs:rounded-xl shadow-2xl border border-gray-200 flex flex-col z-50">
          {/* Header - Mobile First */}
          <div className="bg-blue-600 text-white p-3 xs:p-4 rounded-t-lg xs:rounded-t-xl flex items-center gap-2 xs:gap-3">
            <div className="relative flex-shrink-0">
              <Bot className="w-5 h-5 xs:w-6 xs:h-6" />
              <div className={`absolute -top-1 -right-1 w-2.5 h-2.5 xs:w-3 xs:h-3 ${isConnected ? 'bg-green-400' : 'bg-red-400'} rounded-full`}></div>
            </div>
            <div className="flex-1 min-w-0">
              <h3 className="font-semibold text-sm xs:text-base truncate">IA Eclusa</h3>
              <p className="text-xs text-blue-100 truncate">
                {isConnected ? '🟢 Online' : '🔴 Offline'} • Tempo real
              </p>
            </div>
          </div>

          {/* Messages - Mobile Optimized */}
          <div className="flex-1 overflow-y-auto p-2 xs:p-3 md:p-4 space-y-2 xs:space-y-3" style={{ scrollbarWidth: 'thin' }}>
            {messages.map((message) => (
              <div
                key={message.id}
                className={`flex ${message.isBot ? 'justify-start' : 'justify-end'}`}
              >
                <div className={`flex items-start gap-1.5 xs:gap-2 max-w-[85%] xs:max-w-[80%] ${message.isBot ? '' : 'flex-row-reverse'}`}>
                  <div className={`p-1.5 xs:p-2 rounded-full flex-shrink-0 ${message.isBot ? 'bg-blue-100' : 'bg-gray-100'}`}>
                    {message.isBot ? <Bot className="w-3 h-3 xs:w-4 xs:h-4 text-blue-600" /> : <User className="w-3 h-3 xs:w-4 xs:h-4 text-gray-600" />}
                  </div>
                  <div
                  ...
                  >
                    <p className="text-xs xs:text-sm whitespace-pre-line break-words">{message.text}</p>
            ...
            {isTyping && (
              <div className="flex justify-start">
                <div className="flex items-center gap-1.5 xs:gap-2">
                  <div className="p-1.5 xs:p-2 rounded-full bg-blue-100">
                    <Bot className="w-3 h-3 xs:w-4 xs:h-4 text-blue-600" />
                  </div>
                  <div className="bg-gray-100 p-2 xs:p-3 rounded-lg">
                    <div className="flex gap-1">
                      <div className="w-1.5 h-1.5 xs:w-2 xs:h-2 bg-gray-400 rounded-full animate-bounce"></div>
                      <div className="w-1.5 h-1.5 xs:w-2 xs:h-2 bg-gray-400 rounded-full animate-bounce" style={{ animationDelay: '0.1s' }}></div>
          ...
          {/* Input - Mobile First */}
          <div className="p-2 xs:p-3 md:p-4 border-t border-gray-200">
            <div className="flex gap-1.5 xs:gap-2">
              <input
                ...
                placeholder="Digite: status, alertas, niveis..."
                className="flex-1 px-2 xs:px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs xs:text-sm"
              />
              <button
                onClick={handleSendMessage}
                className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white p-2 rounded-lg transition-colors flex-shrink-0"
              >
                <Send className="w-3.5 h-3.5 xs:w-4 xs:h-4" />
              </button>
            </div>
            <p className="text-xs text-gray-500 mt-1.5 xs:mt-2 flex items-center gap-1 truncate">
              <Bot className="w-2.5 h-2.5 xs:w-3 xs:h-3 flex-shrink-0" />
              <span className="hidden xs:inline">🚀 status • alertas • niveis • portas</span>
              <span className="xs:hidden">🚀 comandos IA</span>
            </p>
          </div>
        </div>
      )}
